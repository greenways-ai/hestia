...
The canonical durable objects are HCV0 cells in `gw_ledger`. JSON projections
...
The browser implementation now constructs native HCV0 records rather than
signing JSON. Each record kind has a fixed ordered schema, uses the stable HCV0
record type tag `14`, and represents every field as another canonical HCV0 root.
Optional values use the canonical nil cell. Maps are ordered by complete HCV0 key
...
2. an Ed25519 signature over the domain-separated bytes
   `GWAR0:<record-kind>:<body-root>`; and
3. a `ledger/signed-record` HCV0 record containing the body root, signer-key
   root, and detached-signature root.

The browser emits a deterministic HCP0 pack for the cells it creates. A reference
...
root. The browser preview hashes the content, builds and signs an HCV0 version
...
human-approval root, signs an HCV0 acceptance record, verifies both offer and
...
2. build, sign, pack, and verify an HCV0 profile version and operational
   delegation;
3. create a HAL-governed private room and AES-GCM epoch;
4. sign a one-time HCV0 invitation with a fragment capability;
5. create an external profile and verify its HCV0 admission proof;
6. rotate the membership epoch;
7. sign and attach an HCV0 document version;
8. encrypt, sign, verify and open a private message;
9. sign an exact-root HCV0 offer; and
10. create canonical human approval and sign exact-root HCV0 acceptance.
...
- PostgreSQL Ed25519 and delegation verification over `GWAR0` bytes;
- reconstruction and validation of every referenced HCV0 cell before admission;
...
HCP0 pack and detached signature, verify the profile and delegation, execute the
