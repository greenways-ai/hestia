...
1. **Contributor intent is signed.** A contributor signs an exact HCV0 batch
...
3. **Receipts are database-planned.** The environment signer receives only the
   `GWDP0` bytes returned by `document_batch_prepare`; it never signs a receipt
...
Their HCV0 roots and signed records are authoritative. A backup or replica may
