...
pub fn sync_lock(project: &Project, mode: LockMode) -> Result<PathBuf, String> {
    ...
    match mode {
        ...
        LockMode::Default | LockMode::Offline => {
            fs::write(&lock, "{:lock/format 1 :packages {}}\n")
...
mod tests {
    ...
    #[test] fn scaffolds_discovers_and_edits_dependencies() { let root = temp("app"); let project = new_app(&root, "hello-app").unwrap(); assert_eq!(discover(&root.join("src/hello_app")).unwrap().id, "hello-app"); set_dependency(&project, "hara/graph", Some("^1.2.0")).unwrap(); assert_eq!(read(&root).unwrap().dependencies["hara/graph"], "^1.2.0"); set_dependency(&project, "hara/graph", None).unwrap(); assert!(read(&root).unwrap().dependencies.is_empty()); fs::remove_dir_all(root).unwrap(); }
