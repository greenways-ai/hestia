...
pub const MANIFEST_SOURCE: &str = include_str!("../../specs/cli/draft/hara-cli.edn");
...
mod tests {
    ...
    fn shared_outcome_conformance_cases_pass() {
        let document = parse(include_str!(
            "../../specs/cli/draft/conformance/outcomes.edn"
        ))
        .unwrap();
    ...
    fn shared_route_conformance_cases_pass() {
        let document = parse(include_str!("../../specs/cli/draft/conformance/routes.edn")).unwrap();
        for case in vector(map_get(&document, "conformance/cases").unwrap()).unwrap() {
