...
fn bigdecimal_hash(words_be: &[u32], signum: i32, scale: i32) -> i32 {
    ...
    let mag: u128 = words_be.iter().fold(0u128, |acc, w| (acc << 32) | (*w as u128));
    ...
    if mag < (1u128 << 63) {
        ...
        let signed = if signum < 0 { temp.wrapping_neg() } else { temp };
...
mod tests {
    ...
    fn element_value(form: &Form) -> Value {
        match form {
            ...
            Form::Vector(items) => {
                Value::Vector(items.iter().map(element_value).collect())
            }
    ...
    /// normative corpus specs/01-lang/020-data-structures/draft/conformance/
    /// hash-parity.edn (generated by target/hashdump/HashDump.java).
    #[test]
    fn java_parity_fixture() {
        let Some(path) = corpus_path(
            "specs/01-lang/020-data-structures/draft/conformance/hash-parity.edn",
        ) else {
            eprintln!(
