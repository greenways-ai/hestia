...
pub fn config_root() -> PathBuf {
    if let Some(root) = env::var_os("HARA_CONFIG_HOME") { return PathBuf::from(root); }
    if let Some(root) = env::var_os("XDG_CONFIG_HOME") { return PathBuf::from(root).join("hara"); }
    env::var_os("HOME").map(PathBuf::from).unwrap_or_else(|| PathBuf::from(".")).join(".config/hara")
}
...
pub fn load(root: &Path) -> Result<BTreeMap<String, Tap>, String> {
    ...
    let Some(taps) = lookup(entries, "taps") else { return Ok(BTreeMap::new()); };
...
pub fn initialize(name: &str, registry: &Path, identity: &Path, root_key: &str) -> Result<InitializedTap, String> {
    if !valid_name(name) { return Err("tap name must contain only lowercase letters, numbers, and hyphens".into()); }
    let root_key = read_hex(root_key, "identity root public key")?;
    if root_key.len() != 32 { return Err("identity root public key must be 32-byte Ed25519 hex".into()); }
...
pub fn initialize_signed(name: &str, registry: &Path, identity: &Path, root_key: &[u8], policy: &str, signature: &str) -> Result<InitializedTap, String> {
    if !valid_name(name) { return Err("tap name must contain only lowercase letters, numbers, and hyphens".into()); }
    if root_key.len() != 32 { return Err("identity root public key must be 32 bytes".into()); }
    verify(root_key, policy.as_bytes(), signature)?;
    ...
    fs::write(registry.join("registry.edn"), registry_document(name, identity, &root_key)).map_err(io)?;
...
pub fn fetch_verified_policy(tap: &Tap, scratch: &Path) -> Result<IdentityPolicy, String> {
    ...
    Ok(IdentityPolicy { revision, publisher_keys })
}

pub fn authorize(policy: &IdentityPolicy, key_id: &str, coordinate: &str, intent: &[u8], signature: &str) -> Result<(), String> {
    let key = policy.publisher_keys.get(key_id).ok_or_else(|| format!("identity policy does not authorize publisher key: {key_id}"))?;
    if key.revoked { return Err(format!("publisher key is revoked: {key_id}")); }
    if !key.coordinates.iter().any(|candidate| candidate == coordinate) { return Err(format!("publisher key {key_id} is not authorized for {coordinate}")); }
    verify(&read_hex(&key.public_key, "publisher public key")?, intent, signature)
}
...
pub fn sign(intent: &[u8]) -> Result<(String, String), String> {
    let signer = env::var("HARA_SIGNER").map_err(|_| "HARA_SIGNER must name an external signer command".to_owned())?;
    let mut child = Command::new(signer).stdin(Stdio::piped()).stdout(Stdio::piped()).spawn().map_err(|error| format!("cannot start HARA_SIGNER: {error}"))?;
    use std::io::Write;
    child.stdin.as_mut().ok_or("cannot open signer stdin")?.write_all(intent).map_err(|error| format!("cannot write publisher intent to signer: {error}"))?;
    let output = child.wait_with_output().map_err(|error| format!("cannot wait for signer: {error}"))?;
    if !output.status.success() { return Err(format!("external signer failed with {}", output.status)); }
    let response = parse(std::str::from_utf8(&output.stdout).map_err(|_| "signer response must be UTF-8")?).map_err(|error| format!("signer response: {error}"))?;
    let response = map(&response, "signer response must be an EDN map")?;
    Ok((string(required(response, "key/id")?, "signer :key/id")?, string(required(response, "signature")?, "signer :signature")?))
}
...
pub fn git(root: &Path, arguments: impl IntoIterator<Item = impl AsRef<std::ffi::OsStr>>) -> Result<String, String> {
    let output = Command::new("git").arg("-C").arg(root).args(arguments).output().map_err(|error| format!("cannot run git: {error}"))?;
...
pub fn clone_first(mirrors: &[String], destination: &Path, label: &str) -> Result<(), String> {
    ...
    for mirror in mirrors {
        if destination.exists() { let _ = fs::remove_dir_all(destination); }
        let output = Command::new("git").args(["clone", "--depth", "1", mirror]).arg(destination).output().map_err(|error| format!("cannot run git: {error}"))?;
        if output.status.success() { return Ok(()); }
        errors.push(format!("{mirror}: {}", String::from_utf8_lossy(&output.stderr).trim()));
    }
    Err(format!("cannot fetch {label} from any configured mirror: {}", errors.join("; ")))
}
...
fn lookup<'a>(entries: &'a [(Form, Form)], key: &str) -> Option<&'a Form> { entries.iter().find(|(candidate, _)| matches!(candidate, Form::Keyword(value) if value == key)).map(|(_, value)| value) }
fn required<'a>(entries: &'a [(Form, Form)], key: &str) -> Result<&'a Form, String> { lookup(entries, key).ok_or_else(|| format!("missing required key :{key}")) }
...
fn strings(form: &Form, label: &str) -> Result<Vec<String>, String> { match form { Form::Vector(values) => values.iter().map(|value| string(value, label)).collect(), _ => Err(format!("{label} must be a vector of strings")) } }
