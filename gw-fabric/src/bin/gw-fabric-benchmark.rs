...
fn main() {
    let options = options().unwrap_or_else(|error| {
        eprintln!("gw-fabric-benchmark: {error}");
        std::process::exit(2);
    });
...
fn options() -> Result<Options, String> {
    ...
    while let Some(argument) = arguments.next() {
        ...
        match argument.as_str() {
            ...
            "--help" | "-h" => {
                println!("gw-fabric-benchmark [--rooms N] [--tasks N] [--payload BYTES] [--clients N] [--shards N]");
                std::process::exit(0);
            }
            _ => return Err(format!("unknown option: {argument}")),
        }
    }
    if options.payload_bytes > gw_fabric::MAX_REPORT_BYTES {
        return Err(format!(
            "--payload exceeds {} bytes",
            gw_fabric::MAX_REPORT_BYTES
        ));
    }
