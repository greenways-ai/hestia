...
use hara_wasm::extension::ExtensionManifest;
...
struct Artifact {
    ...
    compiled: hara_wasm::wasmtime_provider::CompiledWasmModule,
}
...
impl FabricService {
    ...
    pub fn put_module(&self, manifest_source: &str, module: &[u8]) -> Result<String, String> {
        ...
        let compiled = hara_wasm::wasmtime_provider::CompiledWasmModule::compile(module)?;
...
mod tests {
    ...
    fn content_addressed_module_is_compiled_once_and_instanced_per_session() {
        ...
        let arguments = hara_wasm::hta::encode(&Value::Vector(
        ...
        let result = service
            ...
            .unwrap();
        assert_eq!(hara_wasm::hta::decode(&result).unwrap(), Value::Number(42));
