...
|---|---|---|
| `architecture` | HAL controller, boundaries, project configuration | `scripts/connector/run-code architecture` |
| `browser` | `browser/**` | `scripts/connector/run-code browser` |
| `services` | `services/**`, `cloudflare/**` | `scripts/connector/run-code services` |
| `ledger` | document/database paths and ledger fixtures | `scripts/connector/run-code ledger` |
...
The existing `Verify` and `Document ledger` workflows remain authoritative for full browser/service/site checks, Hara project execution, Docker-backed ledger proofs, and required-check aggregation.

A document or database path may select both `architecture` and `ledger`; preserving the controller boundary and validating the ledger contract are separate obligations.
